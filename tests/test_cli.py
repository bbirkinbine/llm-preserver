...
archive path and no network are ever touched.
"""
...
def test_show_refuses_newer_schema_version(tmp_path):
    ...
    result = runner.invoke(app, ["show", str(archive), "acme/tiny-chat"])
...
def test_show_rejects_malformed_model_id(tmp_path):
    ...
    result = runner.invoke(app, ["show", str(archive), "noslash"])
...
def test_show_rejects_trailing_newline_in_model_id(tmp_path):
    ...
    result = runner.invoke(app, ["show", str(archive), "acme/tiny\n"])
...
def test_show_rejects_traversal_model_id(tmp_path):
    ...
    for hostile in ("../outside", "..%2Foutside", "a/../../outside", "a//etc"):
        result = runner.invoke(app, ["show", str(archive), hostile])
...
def test_show_unknown_model_exits_nonzero(tmp_path):
    ...
    result = runner.invoke(app, ["show", str(archive), "acme/no-such-model"])
